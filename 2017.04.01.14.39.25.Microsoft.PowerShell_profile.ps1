...
Set-Alias   -scope global -name   usort         -value   "C:/PROGRA~3/Git/bin/sort.exe"
...
Set-Alias   -scope global -name   ugrep         -value   "C:/PROGRA~3/Git/bin/grep.exe"
...
function rdtdvsql { # {{{
    ...
    invoke-expression -command  "$sys64L/mstsc.exe /f /v:SqlDvSrvr"
    Set-Location $originalLocation
} # }}}
...
function rdtsql { # {{{
    $originalLocation = $PWD
    setdir "//$domainUsrDir/$uid/Desktop/HomeSkutz/TS"
    invoke-expression -command "$sys64L/mstsc.exe /f /v:SqlPrdSrvr"
    Set-Location $originalLocation
} # }}}
...
function pkpath { # {{{
    ...
    $pfd/Git/share/vim/vim74
    $pfx/putty
    $pfx/SlickEdit Pro 19.0.2
    $pfd/Git/bin
    $pfx/cygwin/bin
    $pfx/MKS Toolkit/bin
    $pfx/MKS Toolkit/bin/X11
    $pfx/MKS Toolkit/mksnt
    $prgdat/Oracle/Java/javapath
    $pfx/Support Tools
    $pfx/AMD APP/bin/x86
    $pfx/Common Files/microsoft shared/Windows Live
    $win/Utils
    $sys32
    $win
    $sys32/Wbem
    $sys32/WindowsPowerShell/v1.0
    $pfx/Windows Live/Shared
    $oracle/ora112/bin
    $pfx/ATI Technologies/ATI.ACE/Core-Static
    $pfx/lotus/Notes
    $win/idmu/common
    $sys32/Windows System Resource Manager/bin
    $pfx/Micro Focus/RUMBA
    $pfx/Micro Focus/RUMBA/System
    $pfx/BMC Software/CONTROL-M EM 6.4.01/Default/bin
    $pfx/Verint/Playback
    $pfx/Hummingbird/Connectivity/14.00/Accessories
    $pfx/Hummingbird/Connectivity/14.00/NFS Maestro
    $pfx/Candle/CASP/toolkit/bin
    $pfx/Candle/common/bin
    $pfx/Microsoft Application Virtualization Client
    $pfx/BMC Software/BladeLogic/NSH/bin
    $pfx/Plantronics/PlantronicsURE
    $pfx/WANdisco/Subversion
    $pfx/TortoiseSVN/bin
    $pfx/apache-maven-3.0.4/bin
    $pfx/apache-maven-3.0.4/bin
    $netLclDsk/notes/data
"@
    ingest $heredoc
    pkp5
}   # }}}
...
"@
    ...
    Invoke-Expression "$sys32/cmd.exe /A /Q /K C:/mingw64/mingw64_shell.bat"

    ingest "PATH;$GLOBAL:PSAV4CEF16EF"
} # }}}

function msys { # {{{
    ingest "PSAV21EBC849;$PATHL"
...
"@
    ...
    Invoke-Expression "$sys32/cmd.exe /A /Q /K C:/mingw64/msys2_shell.bat"

    ingest "PATH;$GLOBAL:PSAV21EBC849"
} # }}}

function pull_dvlna253 { # {{{
    $host = "dvlna253"
    $whereiwuz = currentdirectory
    set-location c:/users/nomad/00/nx/$host
...
"@
    ...
    foreach ( $entry in $(sunder $eList) )
    {
        try {
            $element = "{0}:{1}" -f $host, $entry 
            pscp $element  .
        }
        catch {
                $msg = "OUCH:<pull_dvlna253>[ {0}:{1} ] " -f $host,$entry
                $msg += "APPEARS TO BE UNAVAILABLE"
                Write-Host
                Write-Host -ForegroundColor Red $msg
                Write-Host
        }
...
function pulltiv { # {{{
    $whereiwuz = currentdirectory
    set-location c:/users/nomad/00/nx/pssva062
    pscp -r pssva062:.ssh .
    pscp -r pssva062:.tiv .
    pscp -r pssva062:vimfiles .
    pscp -r pssva062:tivoli .
    pscp -r pssva062:bin .
    pscp -r pssva062:vimbk .
    setdir $whereiwuz
} # }}}

function pullnxenv { # {{{
    $whereIwuz = currentDirectory
    plink pvlva614 ksh envpack
    pscp pvlva614:envbase.cpio C:/Users/nomad/00/nx/pvlva614
    pscp pvlva614:envpack C:/Users/nomad/00/nx/pvlva614
    pscp pvlva614:envunpack C:/Users/nomad/00/nx/pvlva614
    setdir "C:/users/nomad/00/nx/pvlva614"
    tar -xf envbase.cpio
    write-output ""
    ll C:/users/nomad/00/nx/pvlva614
    setdir $whereIwuz
} # }}}
...
function pushnxenv { # {{{
    $whereIwuz = currentDirectory
    setdir "C:/users/nomad/00/nx/pvlva614"
    foreach($hostname in $(ql $args)) {
        write-output "%%%%%%%%%%%%%%%%%%%%%%%%%%%%% $hostname %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
        try {
            psftp -b bputenv $hostname
        }
        catch   {
            write-Host -Foregroundcolor yellow $hostname psftp failed
        }
        try {
            plink $hostname ksh envunpack
        }
        catch   {
            write-Host -Foregroundcolor yellow $hostname plink failed
        }
    }
    setdir $whereIwuz
} # }}}
...
function nxgui { # {{{
    ...
    if ( "x86" -eq $ENV:PROCESSOR_ARCHITECTURE.tolower() ) {
        ePlant osbit 32
        $progdir = shortname "$pfx/NoMachine Enterprise Client"
    }
    else {
        ePlant osbit 64
        $progdir = shortname "$pfd/NoMachine Enterprise Client"
    }

    Set-Location $progdir

    $localDir   = "$ENV:UserProfile/Documents/NoMachine"
    $prodnxs    =     "PRODUCTION Linux Desktop on prdlnxgui.vanguard.com.nxs"
    $nonprodnxs = "NON PRODUCTION Linux Desktop on nonprdlnxgui.np-vanguard.com.nxs"
    $authFile   = "NON PROD AUTH1 Linux Desktop on auth1nplnxgui.np-vanguard.com.nxs"

    if ( -not ( Test-Path -path $localDir -PathType container ) ) {
        New-Item -itemtype directory -path $localDir
    }

    if ( -not ( Test-Path -path "$localDir/$nonprodnxs" -PathType leaf ) ) {
       Copy-Item -Force "$progdir/$nonprodnxs" $localDir
    }

    if ( -not ( Test-Path -path "$localDir/$nonprodnxs" -PathType leaf ) ) {
       Copy-Item -Force "$progdir/$prodnxs" $localDir
    }

    if ( -not ( Test-Path -path "$localDir/$authFile" -PathType leaf ) ) {
       Copy-Item -Force "$progdir/$authFile" $localDir
    }

    $cmd2run =  "start-process -windowstyle normal -filepath $progdir/bin/nxplayer.exe"
    Invoke-Expression -command $cmd2run
...
$GLOBAL:initialDirectory = "c:/users/nomad"
...
$GLOBAL:sundryLclD = bs2s "$homeLcl/sundry"
if ( -not ( Test-path -path $sundryLclD -PathType container ) ) {
  new-item -type directory -path $sundryLclD
}
if ( -not ( Test-path -path "$sundryLclD/temp" -PathType container ) ) {
  new-item -type directory -path "$sundryLclD/temp"
}
...
switch -regex ( $localhost ) { # {{{
  '(?six: \A (FultonJSheen) | (Desales) | (Azariah) | (Magdalene) | (T39247)\z )' {
    ...
    $GLOBAL:psDiaryRecent = bs2s "$psDiaryLclD/recent"
    if ( -not ( Test-Path -path $psDiaryRecent -PathType container ) ) {
      New-Item -itemtype directory -path $psDiaryRecent
    }
    $GLOBAL:psDiaryArchive = bs2s "$psDiaryLclD/archive"
    if ( -not ( Test-Path -path $psDiaryArchive -PathType container ) ) {
      New-Item -itemtype directory -path $psDiaryArchive
    }
...
if ( $localhost -match "(?six: (T39247) \z )" ) {
    #sow vimBackupDir '\\vanguard.com\ENTDFS\HOME41\NOMAD\vimbk'
    #sow vimbk "$vimBackupDir"
    sow vimBackupDir "$homelcl/vimbk"
    sow vimbk "$vimBackupDirL"
}
...
sow mgw     "C:/MinGW"
sow mgw     "C:/mingw64"
sow mgw     "C:/MinGW64/x86_64-4.9.3-posix-seh-rt_v4-rev1/mingw64"
sow sqlite  "C:/sqlite"
sow bin     "C:/bin"
...
switch -regex ( $localhost ) { # {{{
  '(?six: \A (FultonJSheen) | (Desales) | (Azariah) | (Magdalene) | (T39247) \z )' {
    sow vimLcl ( ,"$homeLcl" )
    sow VIM "$pfd/Git/share/vim"
    sow VIMRUNTIME "$homelcl/vim/vim74"
  }
...
ePlant git_editor $crunchyvimexe
